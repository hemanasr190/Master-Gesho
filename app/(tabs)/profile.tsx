...
import { Image as AvatarImage } from 'expo-image';
...
import {
  registerForPushNotifications, saveNotificationPrefs, getNotificationPrefs,
  scheduleDailyChallengeReminder, cancelDailyChallengeReminder,
...
export default function ProfileScreen() {
  ...
  const [pushNotifPrefs, setPushNotifPrefs] = useState({ messages: true, suggestions: true, challenges: true });

  // Load push notification prefs and register token
  useEffect(() => {
    if (!user?.id) return;
    getNotificationPrefs(user.id).then(prefs => setPushNotifPrefs(prefs));
    // Register for push notifications silently
    registerForPushNotifications(user.id).catch(() => {});
  }, [user?.id]);

  const handlePushPrefToggle = useCallback(async (key: 'messages' | 'suggestions' | 'challenges', val: boolean) => {
    if (!user?.id) return;
    const next = { ...pushNotifPrefs, [key]: val };
    setPushNotifPrefs(next);
    Haptics.selectionAsync();
    await saveNotificationPrefs(user.id, next);
  }, [pushNotifPrefs, user?.id]);
  ...
  const platformFeatures = [
    {icon:'auto-awesome' as const,label:'استوديو الذكاء الاصطناعي',desc:'إنشاء صور ومحادثات بالذكاء',action:()=>router.push('/ai-studio'),color:'#8B5CF6'},
    {icon:'message' as const,label:'الرسائل',desc:'تواصل مع المجتمع',action:()=>router.push('/messages'),color:'#3B82F6'},
    {icon:'perm-media' as const,label:'مركز الوسائط',desc:'صور وملفات ووثائق',action:()=>router.push('/media-center'),color:'#10B981'},
    {icon:'lightbulb' as const,label:'الاقتراحات',desc:'شارك أفكارك وأيد الآخرين',action:()=>router.push('/suggestions'),color:'#F59E0B'},
    {icon:'history' as const,label:'سجل النشاط',desc:'تاريخ تفاعلاتك',action:()=>router.push('/activity-log'),color:'#06B6D4'},
    {icon:'monetization-on' as const,label:'نقاطي',desc:'مكافآت وإنجازات',action:()=>router.push('/points'),color:'#EC4899'},
    {icon:'cloud-done' as const,label:'حالة الخدمات',desc:'حالة تشغيل المنصة',action:()=>router.push('/service-status'),color:'#22C55E'},
    {icon:'help' as const,label:'مركز المساعدة',desc:'أسئلة وتذاكر الدعم',action:()=>router.push('/help-center'),color:'#7C3AED'},
    {icon:'workspace-premium' as const,label:'الاشتراكات',desc:'الخطط والمزايا',action:()=>router.push('/subscription'),color:'#F97316'},
    {icon:'library-books' as const,label:'مركز البروبمت',desc:'استعرض وأنشئ بروبمتات',action:()=>router.push('/prompt-center'),color:'#14B8A6'},
    {icon:'flag' as const,label:'الإبلاغ',desc:'بلّغ عن محتوى مخالف',action:()=>router.push('/reports'),color:'#EF4444'},
  ];
      ...
      <ScrollView contentContainerStyle={{paddingBottom:insets.bottom+16}} showsVerticalScrollIndicator={false}>
          ...
          <Pressable onPress={() => { Haptics.selectionAsync(); setShowQR(true); }} style={{ position: 'relative' }}>
            {profileAvatarUrl ? (
              <AvatarImage
                ...
                style={[s.avatarContainer, { borderRadius: 40 }]}
                contentFit="cover"
                transition={200}
              />
            ) : (
        ...
        {activeTab==='badges'&&(
          <View style={{paddingHorizontal:16,paddingTop:16,gap:16}}>
            ...
            <View style={{ marginTop: 16 }}>
              <Text style={[s.sectionLabelMini, { color: theme.textMuted, marginBottom: 8 }]}>إشعارات فورية</Text>
              <View style={[s.settingsGroupCard, { marginBottom: 0, padding: 4 }]}>
                {([
                  { key: 'messages' as const, icon: 'message', label: 'الرسائل الجديدة', desc: 'عند وصول رسالة جديدة' },
                  { key: 'suggestions' as const, icon: 'lightbulb', label: 'تصويتات الاقتراحات', desc: 'عند وصول الاقتراح لمعلم' },
                  { key: 'challenges' as const, icon: 'emoji-events', label: 'تذكير التحديات', desc: 'تذكير يومي صباحي' },
                  ...
                  <React.Fragment key={item.key}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', padding: 12, gap: 12 }}>
                      <View style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: theme.backgroundSecondary, alignItems: 'center', justifyContent: 'center' }}>
                        <MaterialIcons name={item.icon as any} size={18} color={theme.textSecondary} />
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 14, fontFamily: 'Cairo_600SemiBold', color: theme.textPrimary }}>{item.label}</Text>
                        <Text style={{ fontSize: 11, fontFamily: 'Cairo_400Regular', color: theme.textMuted, marginTop: 1 }}>{item.desc}</Text>
                      </View>
                      <Switch
                        value={pushNotifPrefs[item.key]}
                        onValueChange={val => handlePushPrefToggle(item.key, val)}
                        trackColor={{ false: '#CBD5E1', true: theme.primary + '60' }}
                        thumbColor={pushNotifPrefs[item.key] ? theme.primary : '#F8FAFC'}
                      />
                    </View>
                  </React.Fragment>
                ))}
        ...
        {activeTab==='activity'&&(
          <>
            ...
            {/* Platform Features */}
            <View style={s.settingsGroup}>
              <Text style={s.groupTitle}>ميزات المنصة</Text>
              <View style={s.settingsGroupCard}>
                  ...
                  <React.Fragment key={item.label}>
                    ...
                    <Pressable style={s.settingsItem} onPress={item.action}>
                      ...
                      <View style={s.settingsInfo}>
                        ...
                        <Text style={s.settingsDesc}>{item.desc}</Text>
                      </View>
                      <MaterialIcons name="chevron-left" size={22} color={theme.textMuted} />
                    </Pressable>
                  </React.Fragment>
